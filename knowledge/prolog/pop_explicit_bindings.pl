...
% Partial Order planner with explicit binding constraints 
...
:- prolog_debug:debug(intermediate_plans, 	true).
...
% :-
%	\+ choosePrecondition(Plan, _, _)
%,	!
%,	prolog_debug:debug(operator_selection, 'No more open preconditions.', [])
%.

%findPlan(_, _, Plan, Plan) :-
%	prolog_debug:debug(plan_control, 'No plan found.', []), !.
...
, 	!   % CUT, no choice points for true preconditions. If we cannot find an operator for this condition, 
		% findPlan fails on this depth and the thing backtracks.
%,	prolog_debug:debug(flaws, 'Positive precondition not achieved', [])
.
...
,	achieved_subcondition(SubCondition, Step, _, Links)
,	!
%,	prolog_debug:debug(flaws, 'Negative precondition already achieved', [])
...
achieved_subcondition(Cond, Step, AchievingStep, Links ) :-
	Link = (AchievingStep, Step, Cond, Unif)  % Unify link with a link between given step and an achieving step.
,	bind(Unif)
...
%; chooseSubgoal(Max, Character, CurrentPlan, AskStepname, PosCondition, NegCondition, NewPlan)
...
,	Step = (_, Op)
,	effectMember(Condition, Op, Bindings, Links)
,	add_to_set((Step,AskStep), Orderings, Orderings2)
,	add_to_set((Step, AskStep, Condition, Bindings), Links, Links2)
...
,	effectMember(Condition, NewOperator, Bindings, Links)
...
,	add_to_set((NewStep, AskStep, Condition, Bindings), Links, Links2)
...
,	schema_agens(NewOperator, Character) % Set agens of chosen schema to given character
,	effectMember(Condition, NewOperator, Bindings, Links) % See if chosen schema is applicable to precondition to search for
,	Cost1 is Cost + 1
,	Cost1 =< MaxCost
,	gensym(step, Name)
,	NewStep = (Name, NewOperator) % Make new step
,	add_to_set((NewStep,AskStep), Orderings, Orderings2) % Add orderings to plan
,	add_to_set((NewStep, AskStep, Condition, Bindings), Links, Links2) % Add links to plan
,	NewPlan = ([NewStep| Steps], Orderings2, Links2, (Depth1, Cost1, Improvs))
,	ignore((
		debugging(operator_selection)
	,	show_step(AskStep, TxtAsk)
...
,	effectMember(FramingEffect, FramingOperator, Bindings, LinksIn)
,	\+ member((StartStep, FramingStep, FramingEffect), LinksIn)	
,	!
,	append(LinksIn, [(StartStep, FramingStep, FramingEffect, Bindings)], LinksIn2)	
,	add_framing_effects_as_causal_links(FramingStep, LinksIn2, LinksOut)		
.

add_framing_effects_as_causal_links(_, LinksIn, LinksIn).

effectMember(Condition, Operator, Bindings, Links) :-
	operator_effects(Operator, E),
	member(E1,E),
	condition_member(Condition,E1, Bindings),
	compatible_bindings(Bindings, Links)
...
,	operator_effects(Operator, E)
,	member(Em,E)
,   condition_member(E1, Em)
		% This check to see whether two steps are in conflict assumes that the preconditions and effects
		% of the two steps have been fully instantiated. Another implementation might use binding constraints
		% (i.e. the step doesn't threaten as long as Location1 != Location2.

		% ground(+Term) succeeds when Term contains no free variables (SWI-Prolog)

	
% Try out the ground elements of the list to see with which of these ground elements the 
% test element equates to (==). The list may contain non-ground members, and because of 
% equality, it doesn't matter to consider choice points (hence the cut).
...
%	check if Condition == inverse of E1
,	inverse_condition(Condition, E1)
%,	prolog_debug:debug(threats, 'inverse_condition: ~w~n~w~n', [Condition, E1])
,	possible_after(Orderings, (ThreateningStep, Step1))
,	possible_after(Orderings, (Step2, ThreateningStep))
,	ignore((
		debugging(threats)
...
compatible_bindings(_, []).

compatible_bindings(Bindings, [L1|Links]) :-
	L1 = (_,_,_,Bindings2)
,	compatible(Bindings, Bindings2)
,	compatible_bindings(Bindings, Links)
.
